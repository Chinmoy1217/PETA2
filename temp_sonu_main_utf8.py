...
async def predict_eta(
    ...
    try:
        ...
        if len(df) == 1 and pol and pod:
             ...
             if stops_count > 0:
                 explanation += f"ΓÜá∩╕Å Logic: Detected {stops_count} transshipment stop(s) at {via_s}.\n"
        ...
        if accuracy:
            ...
            if deviations > 0:
                summary += f" ΓÜá∩╕Å **{deviations} shipments** show significant deviation (>5h)."
            else:
                summary += " Γ£à All shipments are tracking within expected margins."
        else:
            summary += " Predictions generated successfully."

        if is_training:
            summary += "\n\n≡ƒåò **Continuous Learning Active**: These records have been added to the master database. The model is retraining in the background to improve future accuracy."
